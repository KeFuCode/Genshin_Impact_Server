...
`Drop.csv` 配置表的设置很重要，若不能合理设置，会出现概率不准的情况。
`isMustUp` 控制五星卡牌的大小保底。

测试 100000000 次抽卡，在保底机制的控制下，也很难出现 90 次抽卡都没有获得五星卡的情况。
测试 100000000 次抽卡，在保底机制的控制下，没有出现过 10 连抽五个五星卡，4 个 4 星卡的出现次数只有 4 次。
...
```
# 5 Golang 协程
前面通过在 main.go 中加入无限循环，实现主程序 main 的持续运行。这种情况，只能手动强制关闭主程序。
可以通过 Golang 内置的方法，实现等待协程运行结束后，自动关闭主程序。修改之后的 main.go 程序，只作为程序启动的入口，与业务代码进行了切割。
...
Golang 中，用小写命名的变量，对外部不可见，序列化生成的 Json 文件中，也没有小写变量。
如果需要保存小写声明的变量，使用 `json:""` 可以存储变量，并自定义变量名。
...
# 8 网络通信
OSI 七层模型（应用层、表示层、会话层、传输层、网络层、数据链路层、物理层）只是理论划分，实际使用的是 TCP/IP 五层模型（应用层、传输层、网络层、数据链路层、物理层）。

对于软件开发，打交道最多的是应用层（HTTP、WebSocket）、传输层（TCP、UDP），网络层（IP）很少用，数据链路层和物理层完全不接触。

对于现代编程语言，网络通信相关的内容，都被封装在了语言设计里，可以简单的十几行代码，就能实现网络通信的功能。网络通信不作为工作中的核心和重点，能熟练运用应用层和传输层的知识即可，不需要花费大量精力深入研究。